...
if(isset($_POST["cadastrar"])){
    ...
    $querySelect2 = ("SELECT ProcessActivitie, ResponsabilitiesAudit, PeriodAudit FROM auditplan WHERE idAuditPlan = $idauditplan");
    ...
    $hoje = date('d/m/Y'); 
    $prazo = date('d/m/Y', strtotime('+30 days'));
	...
	$html .= '<th>Critério</th>';
	$html .= '<th>Situação</th>';
    $html .= '<th>Prazo para Resolução da Não Conformidade</th>';
	$html .= '</tr>';
    $html .= '</thead>';
    $html .= '<tbody>';
	...
	// Carrega seu HTML
	$dompdf->load_html('
			<h1 style="text-align: center;">PLANO DE AÇÃO DE AUDITORIA</h1>
			'. $html .'
		');
